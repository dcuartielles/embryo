/*
   ...
   Test 11: Motors Functions Multiple Steps
   For some reason having the call to the pulseMotor function and the timer one
   as well seems to make the magic. This has to be evaluated further but for now
   this is as good as it gets.

   Known things to fix:

   1) add the button to the machine's door in the blocking part of the code
   2) clean the code, 
   ...
   Notes: remember that the machine the students use has not end stop at the 
   bottom. Therefore, code does not support that interaction
...
void setup() {
  ...
  // call the toggle_led function every 500 micros (1 second)
  timer.every(500, pulseMotor);
...
void loop() {
  ...
  if (inputEvent() || currentOperationMode == AUTOMATIC) {
    ...
    if (pinEvent(8)) {
      ...
      changeState(RESET);
      firstCycle = -1; // mark this as a configuration cycle for the automatic mode
    }
...
//      }
    ...
    case AUTO_UP:
      ...
      if (inputValues[0] == HELD && inputValues[1] == HELD &&
        ...
        changeOperationState(AUTO_DOWN);
        stepsCount = 0;
        if (DEBUG) Serial.println(F("** STOP - and DOWN **"));
    ...
    case AUTO_DOWN:
      ...
      // move down for as long as LEFT, and RIGHT are HELD and max steps is not reached
      if (inputValues[0] == HELD && inputValues[1] == HELD &&
