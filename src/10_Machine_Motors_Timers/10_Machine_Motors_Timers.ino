/*
   ...
   Test 10: Motors Functions
   Work with a timer interrupt to deal with the motor movement
   in a smooth way. Implies moving the motor render section to
   a separate interrupt function

   Known things to fix:

   1) the automatic mode has to be debugged, once it goes up, it enters a loop
   2) even with the timer, it makes horrible noises when going down
